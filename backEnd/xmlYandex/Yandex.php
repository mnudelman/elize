...
class Yandex
{
     ...
     */
    protected $key;
    protected $url ;       // добавлено 23.01.2016 - Нудельман
    /**
     * Query
     *
     * @var string
...
//        44 => 'Данный адрес для совершения запросов больше не поддерживается — используйте адрес, выданный вам на странице настроек',
     ...
     */
     ...
     {
         if (empty($user) or empty($key) or empty($url)) {
             throw new Exception('Yandex: username and key and url is requeried');
         }
         $this->user = $user;
         $this->key  = $key;
         $this->url = $url ;
...
//             . '&key='.$this->key;
    ...
    static public function getHighlight($xml)
    {
        // FIXME: very strangely method
        $text = $xml->asXML();
